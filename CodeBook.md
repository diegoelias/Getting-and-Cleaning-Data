...
5. Merges the features with activity labels and subject IDs. The result is
   saved as `tidyDataSet.txt`.

6. The average of each measurement for each activity and each subject is merged
   to a second data set. The result is saved as `tidyDataSet2.txt`.
...
- tidyDataSet - subsetted, human-readable data ready for output according to
  project description.
- uS - unique subjects from S
- nS - number of unique subjects
- nA - number of activities
- nC - number of columns in tidyDataSet
...
`tidyDataSet.txt` is a 10299x68 data frame.
...
`tidyDataSet2.txt` is a 180x68 data frame.
